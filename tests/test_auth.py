...
def test_register(client, app, monkeypatch):
    ...
    monkeypatch.setattr('flaskr.auth.validate_recaptcha_response',
                        mock_validate_captcha_response)
    response = client.post(register_url, data=postdata)
...
def test_password_hash_unique(app):
    ...
    hashes = {row[0] for row in get_db().execute(
        'SELECT password FROM user WHERE username LIKE "example%"'
    ).fetchall()}
    assert len(hashes) == 2
