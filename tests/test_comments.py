...
def test_get_post_comments(app):
    ...
    with app.app_context():
        assert dicts(get_post_comments(1)) == [
            {'id': 1, 'body': 'comment11', 'author_id': 1, 'username': 'test', 'created': datetime(1911, 1, 1)},
            {'id': 2, 'body': 'comment12', 'author_id': 2, 'username': 'other', 'created': datetime(1912, 1, 1)},
        ]
        assert dicts(get_post_comments(2)) == [
            {'id': 3, 'body': 'comment21', 'author_id': 1, 'username': 'test', 'created': datetime(1921, 1, 1)},
            {'id': 4, 'body': 'comment22', 'author_id': 2, 'username': 'other',  'created': datetime(1922, 1, 1)},
        ]
...
def test_pass_through_comments(client, monkeypatch, post_id):
    comments = [
        {'id': 5, 'body': f'comment{post_id}1', 'author_id': 1, 'username': 'test', 'created': datetime(1921, 1, 1)},
        {'id': 6, 'body': f'comment{post_id}2', 'author_id': 2, 'username': 'other',  'created': datetime(1922, 1, 1)},
    ]
...
def test_comments_rendering(app, post_id):
    comments = [
        {'id': 5, 'body': f'comment{post_id}1', 'author_id': 4, 'username': f'test{post_id}', 'created': datetime(1921, 1, 1)},
        {'id': 6, 'body': f'comment{post_id}2', 'author_id': 5, 'username': f'other{post_id}',  'created': datetime(1922, 1, 1)},
    ]
    ...
    assert re.search('.*'.join((
        comments[0]['username'], comments[0]['created'].isoformat(' ', 'minutes'), comments[0]["body"],
        comments[1]['username'], comments[1]['created'].isoformat(' ', 'minutes'), comments[1]["body"],
    )), result, re.DOTALL)
...
def test_add_comment(client, auth, post_id):
    ...
    assert b'Missing comment body' in client.post(f'/{post_id}/comments/new', data={'body': ''}).data
    body = b'newcomment'
    assert client.post(f'/{post_id}/comments/new', data={'body': body}
            ).headers['Location'] == f'http://localhost/{post_id}'
    assert body in client.get(f'/{post_id}').data # FIXME
