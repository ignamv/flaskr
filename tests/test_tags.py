...
def test_update_post_tags_integration(client, tags, app, auth):
    auth.login()
    data = {'title': 'newtit', 'body': 'bod', 'tags': ','.join(tags), 'file': (BytesIO(b''), '')}
...
def test_create_with_tag_integration(client, auth, app):
    auth.login()
    data = {'title': 'created', 'body': 'abody', 'tags': 'tag1,tag2', 'file': (BytesIO(b''), '')}
...
def test_create_with_tag_mocking_insert(client, auth, app, monkeypatch):
    ...
    auth.login()
    tags = ['tag1', 'tag2']
    data = {'title': 'created', 'body': 'abody', 'tags': ','.join(tags), 'file': (BytesIO(b''), '')}
