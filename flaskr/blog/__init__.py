...
def update(post_id):
    ...
    if request.method == 'POST':
        ...
        if tags == ['']:
            tags = []
        imagebytes = request.files['file'].read() or None
        delete_image = {'on': True, 'off': False}[request.form.get(
            'delete_image', 'off')]
...
def like(post_id):
    ...
    if db.execute('SELECT id FROM post WHERE id = ?', (post_id,)
                  ).fetchone() is None:
        abort(404)
    if new_status:
        db.execute('INSERT INTO like (post_id, user_id) VALUES (?, ?)',
                   (post_id, g.user['id']))
    else:
        db.execute('DELETE FROM like WHERE post_id == ? AND user_id == ?',
                   (post_id, g.user['id']))
    db.commit()
...
def post_image(post_id):
    row = get_db().execute('SELECT imagebytes FROM post WHERE id == ?',
                           (post_id,)).fetchone()
