...
from .blogdb import get_post, create_post, update_post, get_posts, count_posts, page_size
...
def update(post_id):
    ...
    if request.method == 'POST':
        ...
        if tags == ['']:
            tags = []
        imagebytes = request.files['file'].read() or None
        delete_image = {'on': True, 'off': False}[request.form.get('delete_image', 'off')]
        ...
        if error is None:
            ...
            flash(error)
    return render_template('blog/new.html',
            post_id=post_id, title=post['title'], body=post['body'],
            tags=','.join(post['tags']))
